...
#
# This module tests the API foundation including types, server endpoints, and client library.

import std/[times, tables, options, json, strutils, asyncdispatch, unittest]
import ../src/api/types
import ../src/api/server
import ../src/api/client
import ../src/types

const RUN_API_TESTS* = true  # Set to false for performance

when RUN_API_TESTS:
  suite "API Foundation Tests":
    setup:
      # Initialize test server
      var server = newApiServer(8081)
      var client = newApiClient("http://localhost:8081", "test_agent")
    
    test "API Types and Constructors":
      ## Test API type constructors and serialization
      
      # Test API response
      let response = newApiResponse("test_data")
      check response.success == true
      check response.data.isSome
      check response.data.get() == "test_data"
      check response.error.isNone
      
      # Test error response
      let error_response = newApiResponse[string]("test_error")
      ...
      let message = newAgentMessage("test_agent", query_request, content, high)
      check message.from_agent == "test_agent"
      check message.message_type == query_request
      check message.priority == high
      check message.content == content
      
      # Test stream event
      let event = newStreamEvent(entity_created, %*{"entity_id": "test_entity"})
      check event.event_type == entity_created
    ...
    test "JSON Serialization":
      ## Test JSON serialization of API types
      
      # Test API response serialization
      let response = newApiResponse("test_data")
      let json_response = response.toJson
      check json_response["success"].getBool == true
      check json_response["data"].getStr == "test_data"
      
      # Test agent message serialization
      let content = %*{"key": "value"}
      let message = newAgentMessage("test_agent", query_request, content)
      let json_message = message.toJson
      check json_message["from_agent"].getStr == "test_agent"
      check json_message["message_type"].getStr == "query_request"
      check json_message["priority"].getStr == "normal"
      
      # Test stream event serialization
      let event = newStreamEvent(entity_created, %*{"entity_id": "test_entity"})
      let json_event = event.toJson
      check json_event["event_type"].getStr == "entity_created"
      check json_event["data"]["entity_id"].getStr == "test_entity"
    ...
    test "Authentication and Authorization":
      ...
      check token.hasPermission(read_entities) == true
      check token.hasPermission(write_entities) == false
      check token.hasPermission(full_access) == false
      
      # Test full access token
      let full_access_token = newAuthToken("admin_agent", @["full_access"], 1)
      check full_access_token.hasPermission(read_entities) == true
      check full_access_token.hasPermission(write_entities) == true
      check full_access_token.hasPermission(delete_entities) == true
    
    test "API Client Basic Operations":
      ## Test basic API client functionality
      
      # Test client creation
      let test_client = newApiClient("http://localhost:8080", "test_agent")
      check test_client.base_url == "http://localhost:8080"
      check test_client.agent_id == "test_agent"
      check test_client.auth_token.isNone
      
      # Test auth token setting
      test_client.setAuthToken("test_token")
      check test_client.auth_token.isSome
      check test_client.auth_token.get() == "test_token"
      
      # Test connection status
      check test_client.isConnected == false  # No requests made yet
    
    test "Error Handling":
      ## Test error handling in API types
      
      # Test API error creation
      let error = newApiError(invalid_request, "Test error message", "req_123")
      ...
      let error_no_id = newApiError(permission_denied, "Permission denied")
    ...
    test "Query Filter System":
      ...
      var populated_filter = QueryFilter()
    ...
    test "Message Priority System":
      ...
      let priorities = @[low, normal, high, critical, emergency]
      
      for priority in priorities:
        let content = %*{"test": "data"}
        let message = newAgentMessage("test_agent", query_request, content, priority)
    ...
    test "Agent Status System":
      ...
      let status_types = @[idle, active, busy, sleeping, error, disabled]
